...
import com.order.model.vo.OrderRetVo;
import lombok.Data;
...
public class Orders implements VoObject, Serializable {
    ...
    private List<com.order.model.bo.OrderItems> orderItemsList;
    ...
    //利用Po更新Bo
    public Orders updateOrders(OrdersPo po) {
        this.id = po.getId();
        this.customerId=po.getCustomerId();
        this.shopId=po.getShopId();
        this.orderSn=po.getOrderSn();
    ...
    //修改非空的属性（买家修改订单API）
    public Orders changeOrders(Orders orders)
    {
        if(orders.getConsignee()!=null) this.setConsignee(orders.getConsignee());
        if(orders.getRegionId()!=null) this.setRegionId(orders.getRegionId());
        if(orders.getAddress()!=null)  this.setAddress(orders.getAddress());
        if(orders.getMobile()!=null) this.setMobile(orders.getMobile());
